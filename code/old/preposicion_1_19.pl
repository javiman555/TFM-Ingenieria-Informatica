% Definición de un conjunto unitario (singleton)
unitario(X, [X]).

% Verifica si todos los conjuntos unitarios {x} están en Tau
...
% Expresa un conjunto S como la unión de sus unitarios {x}
es_union_de_unitarios(S, X, Tau) :-
    mi_subset(S, X), % S es un subconjunto de X
    todos_los_unitarios_en_tau(S, Tau). % Verifica si todos los unitarios de S están en Tau

% Demostración de que Tau es la topología discreta
